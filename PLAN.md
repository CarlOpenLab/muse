...
* ✅ **Agent 工具调用**：侧栏 AI 用自然语言说明文档问题，模型通过 OpenAI function calling 直接修改文档（get\_document / get\_selection / replace\_selection / insert\_at\_cursor / insert\_at\_end / replace\_text 六工具）；
  ipcChatFetch 内 agent loop 逐轮请求 → 流中解析 tool\_calls → dispatchEditorTool 执行（MilkdownCore transaction，⌘Z 可撤销）→ 结果反馈下一轮；
...
* ✅ **@antdv-next/x 组件化**：思考链（深度思考 reasoning\_content 流式 + 工具调用 loading→success|error）用 `ThoughtChain` 渲染（chainStore 模块级数据，agent loop 写入）；
